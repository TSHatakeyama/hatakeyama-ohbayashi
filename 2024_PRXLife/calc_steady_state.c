...
int main(int argc, char *argv[])
{
    ...
    sprintf(filepath, "steady_state_%d_%d_generation.data", init_genome_num, final_generation);
    fp = fopen(filepath, "w");
    ...
    for (seed = 0; seed < 10000; seed++)
    {
        ...
        var_fitness = calc_var(fitness_array, N);
        ...
        for (generation = 1; generation <= GENERATION; generation++)
        {
            ...
            for (i = 0; i < N; i++)
            {
                ...
                pheno_array[i] = indiv[i].pheno;
                fitness_array[i] = indiv[i].fitness;
                if (indiv[i].pheno > 3.441212 || indiv[i].pheno < -11.374583)
                {
                    growth_array[i] = 0.0;
                }
                else
            ...
            {
                fprintf(fp, "%d %f %f %f %f %f %f %f %f %f %f\n", seed, avg_fitness, var_fitness, avg_pheno, var_pheno, avg_var_genome, var_var_genome, avg_skew_genome, var_skew_genome, avg_kurt_genome, var_kurt_genome);
                fflush(fp);
            }
        }
    }
